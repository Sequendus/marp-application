...
```{r, warning=FALSE, message=FALSE}
cb_palette <- c(
  "S1_Gamma_well_specified" = "#0072B2",
  "S2_Weibull_challenging" = "#E69F00",
  "S3_Mixture_misspecified" = "#009E73"
)

ci_failure_summary %>%
  mutate(n = factor(n)) %>%
  ggplot(aes(x = n, y = ci_failure_rate, fill = scenario)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  scale_fill_manual(values = cb_palette) +
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
  labs(
    title = "Failure rate of Studentized bootstrap confidence intervals",
    x = "Sample size",
    y = "CI failure rate",
    fill = "Scenario"
  ) +
...
```{r, warning=FALSE, message=FALSE}
# Optional: since CI failure rates are high
ci_summary %>%
  mutate(
    n = factor(n),
  ...
  ggplot(aes(x = n, y = coverage, colour = method, group = method)) +
  geom_hline(yintercept = 0.95, linetype = "dashed") +
  geom_point(aes(size = n_ci_success)) +
  geom_line(linewidth = 0.8) +
  scale_colour_manual(values = method_palette) +
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
  facet_grid(quantity_label ~ scenario) +
  labs(
    title = "Empirical CI coverage among successful CI runs",
    subtitle = "Coverage is conditional on successful CI computation; point size indicates number of successful CIs",
    x = "Sample size",
    y = "Coverage",
    colour = "Method",
    size = "Successful CIs"
