...
# step one: fitting different renewal models
...
pdf("figures/loghaz_plot_seizure.pdf")
...
# Save R objects
saveRDS(res, "results/seizure/res_seizure.rds")
saveRDS(ci,  "results/seizure/ci_seizure.rds")
...
write.csv(model_summary, "results/seizure/model_summary_seizure.csv", row.names = FALSE)

haz_summary <- data.frame(
  quantile      = c("10%", "36.7%", "63.3%", "90%"),  # 4 quantiles not 7
  t             = t,
...
write.csv(haz_summary, "results/seizure/haz_summary_seizure.csv", row.names = FALSE)
